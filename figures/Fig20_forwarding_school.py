...
from Fig18_by_school import plot_blocks
...
def bar_plot(ACOLE, filename):
    ...
    for i, ACOLE_by_module in enumerate([m1, m2, m3]):
        ...
        schools = [k for k in ACOLE_by_module.schools(True).keys()]

        all_data = [ACOLE_by_module.by_school(ages) for ages in schools]
    ...
    fig.text(0.5, -0.08, 'Escolas', ha='center', va='center', fontsize=12)
    plt.tight_layout()
...
def plot():
    bar_plot(ACOLE1, 'Fig20_escola_encaminhamento')
