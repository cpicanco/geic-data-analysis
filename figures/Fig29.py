        ...
        if len(student.acoles) > 1:
            if len(student.modules) > 0:
                if student.has_m3:
                    ...
                    for block, student_block in zip(MODULE3.blocks, student.modules[0].blocks):
                        for key, data in student_block.data.items():
                            if len(data) > 0:
                                block.data[key].append(data[0])

    bar_plot(ACOLE1, MODULE3, ACOLE2)
    bar_plot(ACOLE1, MODULE3, ACOLE2, use_boxplot=True)